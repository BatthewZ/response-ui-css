...
| [global-defaults.md](./global-defaults.md) | The rules that repaint a browser default (scrollbars, selection, ring offset): why an unlayered one cannot be overridden by a utility, why such a rule should alias an existing token pair instead of minting literals, and how a pseudo-element rule scopes itself. Read before adding a package-wide default. |
