...
| [global-defaults.md](./global-defaults.md) | The rules that repaint a browser default (scrollbars, selection, ring offset): why an unlayered one cannot be overridden by a utility, why such a rule should alias an existing token pair instead of minting literals, how a pseudo-element rule scopes itself, and why an alias's default belongs at the read rather than in a `:root` declaration if it is to survive a theme scoped to a subtree. Read before adding a package-wide default. |
